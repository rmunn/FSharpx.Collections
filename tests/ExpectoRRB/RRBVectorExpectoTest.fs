module ExpectoRRB.RRBVectorExpectoTest
...
let mergeTests =
  testList "merge tests" [
    ...
    testCase "adjustTree is needed in merge algorithm when tree height does not increase" <| fun _ ->
        ...
        let fullNode = FSharpx.Collections.Node(ref null, [|1..Literals.blockSize|] |> RRBHelpers.arrayToObjArray)
        let fullNodeMinusOne = FSharpx.Collections.Node(ref null, [|1..Literals.blockSize-1|] |> RRBHelpers.arrayToObjArray)
...
let experimentalTests =
  testList "Experimental tests" [
    testProp "append and pop" <| fun (vec : RRBVector<int>) ->
        ...
        FsCheck.Experimental.StateMachine.toProperty (ExpectoRRB.ExperimentalFsCheckCommands.VecCommands.machine vec)
  ]
...
let main argv =
        ...
        // Expect.equal (RRBVector.length vec) 9 "9-item vector should have length 9"
        // 0
    // let vec = RRBVecGen.treeReprStrToVec "2 4 M T4"
    // RRBVectorProps.checkProperties vec "Original vector"
    // let vL, vR = vec |> RRBVector.split 2
    // RRBVectorProps.checkProperties vL "Left split result"
    // RRBVectorProps.checkProperties vR "Right split result"
    // printfn "left vec %A passed checks" vL
    // printfn "right vec %A passed checks" vR
    // let join = RRBVector.append vL vR
    // printfn "checking joined vec %A ..." join
    // RRBVectorProps.checkProperties join "Joined result"
    // printfn "joined vec %A passed checks" join
    // 0
    ...
    // let revL = RRBVector.rev vL
    // let revR = RRBVector.rev vR
    // RRBVectorProps.checkProperties revL "Left split+rev result"
    // RRBVectorProps.checkProperties revR "Right split+rev result"
    // printfn "rev left vec %A passed checks" revL
    // printfn "rev right vec %A passed checks" revR
    // let join = RRBVector.append revR revL
    // printfn "checking joined vec %A ..." join
    // RRBVectorProps.checkProperties join "Joined vector"
    // printfn "joined vec %A passed checks" join
    // Expect.vecEqual join (RRBVector.rev vec) "Vector halves after split+reverse, when put back together, did not equal reversed vector"

        // let vec = ridiculouslyBigVector
        // RRBVectorProps.checkProperties vec "Original huge vector"
        // let vL, vR = vec |> RRBVector.split 9  // Past first leaf, but not very far past
        // RRBVectorProps.checkProperties vL "Left side of split"
        // RRBVectorProps.checkProperties vR "Right side of split"
        ...
        // let arrTotal = vec |> RRBVector.toArray
        // Expect.equal (Array.append arrL arrR) arrTotal "Items in vectors after split did not add up to original vector's items"
        // printfn "Right side (big): %A" vR
        // printfn "Left side (small): %A" vL
        // let revL = vL |> RRBVector.rev
        // let revR = vR |> RRBVector.rev
        // RRBVectorProps.checkProperties revL "Rev left side of split"
        // RRBVectorProps.checkProperties revR "Rev right side of split"
        // printfn "Rev right side (big): %A" revR
        // printfn "Rev left side (small): %A" revL
        // // let vLpop = vL |> RRBVector.pop
        // // RRBVectorProps.checkProperties vLpop "Left side, popped"
        // // printfn "Left side (popped): %A" vLpop
        // let vMerged = RRBVector.append vL vR
        // RRBVectorProps.checkProperties vMerged "vMerged"
        // let revMerged = RRBVector.append revR revL
        // RRBVectorProps.checkProperties revMerged "revMerged"
        // Expect.equal (RRBVector.toArray revMerged) ((RRBVector.rev >> RRBVector.toArray) vMerged) "Items in vectors after rev"
        // // printfn "vMerged before check: %A" vMerged
        // // printfn "... Looks like it passed the test."
        // 0

        // let v = RRBVecGen.treeReprStrToVec "[M M M M M M M M M 30 M-1 M M-1 M 30 M] [M M-1 M M M-1 M M M M 28 30 M M 29 M] [27 25 26 M-1 M-1 26 26 26 27 27 25 28 27 24 22 M] T8"
        // let v' = v.Insert 0 -512
        // RRBVectorProps.checkProperties v' "Inserted vector"
        // 0
