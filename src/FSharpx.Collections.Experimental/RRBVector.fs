// Search strings to look for: TODO, TOCHECK, XYZZY
...
module rec FSharpx.Collections.Experimental.RRBVector

open System.Threading
open RRBArrayExtensions
open FSharpx.Collections  // Since we need to inherit from the Node class from PersistentVector
...
module internal Literals =
    ...
    let [<Literal>] internal blockBitMask = 31  // 2 ** blockSizeShift - 1. Use with &&& to get the current level's index
...
module RRBHelpers =
    ...
    let inline radixIndex shift treeIdx =
        (treeIdx >>> shift) &&& Literals.blockBitMask
    ...
    let ofPersistentVector (vec:FSharpx.Collections.PersistentVector<'T>) =
...
module RRBVector =
    ...
    let inline ofSeq (s : seq<'T>) = FSharpx.Collections.PersistentVector.ofSeq s |> RRBHelpers.ofPersistentVector
